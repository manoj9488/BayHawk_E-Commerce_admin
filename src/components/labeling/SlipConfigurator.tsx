...
interface SlipConfiguratorProps {
  ...
  className?: string;
  previewValues?: Record<string, string | number | null | undefined>;
}

export const SlipConfigurator = ({ config, onChange, className, previewValues }: SlipConfiguratorProps) => {
  ...
  const [activeTemplateId, setActiveTemplateId] = useState("delivery-template");

  useEffect(() => {
    const shouldBootstrap =
      templates.length === 0 || templates.every((template) => template.fields.length === 0);
    ...
    setTemplates([
      ...
      },
      {
        ...
        name: "Packing Slip",
        type: "packing",
        fields: config.packingSlip,
        createdAt: now,
        updatedAt: now,
      },
    ]);
  }, [config.deliverySlip, config.packingSlip, templates]);
