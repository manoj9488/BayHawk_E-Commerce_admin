...
export function CompleteProductForm({
  onSave,
  onCancel,
  initialData,
  moduleScope,
}: CompleteProductFormProps) {
  ...
  const [availableCuttingTypes, setAvailableCuttingTypes] = useState<CuttingTypeOption[]>([]);
  ...
  useEffect(() => {
    ...
    const loadLookups = async () => {
      ...
      try {
        const [categories, cuttingTypes, products] = await Promise.all([
          listProductCategories(moduleScope),
          listCuttingTypes(moduleScope),
          listProducts({
            moduleScope,
            limit: '100',
          }),
        ...
        setAvailableCuttingTypes(cuttingTypes);
        setAvailableProducts(products.filter((product) => product.id !== (initialData as { id?: string } | undefined)?.id));
      } catch (error) {
        if (!cancelled) {
          console.error('Failed to load product form lookups', error);
  ...
  const handleImageUpload = (images: Array<File | string>) => {
    setFormData(prev => ({
      ...prev,
      images,
  ...
  return (
    <div className="space-y-6 max-h-[80vh] overflow-y-auto">
      ...
      {isLoadingLookups && (
        <Card>
          <p className="text-sm text-gray-600">
            Loading categories, cutting types, and combo product references...
          </p>
        </Card>
      )}
        ...
        <div className="space-y-6">
          {formData.variants.map((variant, index) => (
            <ProductVariantForm
              ...
              variant={variant}
              index={index}
              availableCuttingTypes={availableCuttingTypes.map((cuttingType) => ({
                id: cuttingType.id,
                name: cuttingType.name,
                description: cuttingType.description || '',
                category: cuttingType.category,
              }))}
