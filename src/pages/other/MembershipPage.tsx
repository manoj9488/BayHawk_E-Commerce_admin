...
import { membershipPlansApi } from '../../utils/api';
...
export function MembershipPage() {
  ...
  const [apiPlans, setApiPlans] = useState<typeof mockPlans>([]);
    ...
    const loadPlans = async () => {
      try {
        ...
        const mapped = plans.map((plan: any) => ({
          id: plan.id,
          name: plan.name,
          price: Number(plan.priceAmount || 0),
          billingCycle: Number(plan.durationDays || 0) >= 365 ? 'yearly' : 'monthly',
          members: 0,
          features: Array.isArray(plan.benefits) && plan.benefits.length > 0
            ? plan.benefits.map((item: any) => item.title)
            : [`${plan.extraDiscountPercent || 0}% extra discount`, 'Priority support'],
  ...
  const visiblePlans = useMemo(() => {
    return apiPlans.length > 0 ? apiPlans : mockPlans;
  }, [apiPlans]);
      ...
      <div className="grid gap-6 sm:grid-cols-4">
        ...
        <Card className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold">{visiblePlans[0]?.members ?? 0}</p>
          </div>
        </Card>
        <Card className="flex items-center gap-4">
