...
export function HubPage() {
  ...
  useEffect(() => {
    ...
    const loadHubs = async () => {
      try {
        ...
        if (!cancelled && Array.isArray(hubsData)) {
          setHubs(hubsData);
        }
      } catch (error) {
        if (!cancelled) {
  ...
  const handleSaveHub = async (hubData: Partial<Hub>) => {
    if (editingHub) {
      try {
        const response = await hubsApi.update(editingHub.id, hubData);
        const updatedHub = response?.data?.data;

        setHubs((prev) =>
          prev.map((hub) =>
            hub.id === editingHub.id
              ? (updatedHub as Hub) || { ...hub, ...hubData, id: editingHub.id }
              : hub,
          ),
        );
      ...
      try {
        const response = await hubsApi.create(hubData);
        const createdHub = response?.data?.data;

        if (createdHub) {
          setHubs((prev) => [...prev, createdHub as Hub]);
        }
